...
Three groups of techniques are interdependent: \texttt{T2.1}~(\texttt{local}, RWX in one step) pairs with \texttt{T4.1}~(plain \texttt{memcpy}); \texttt{T2.2}~(\texttt{local\_rw}, RW only) pairs with \texttt{T4.2}~(\texttt{memcpy} followed by a flip to RX); and \texttt{T2.3}~(\texttt{remote}) pairs with \texttt{T4.3}~(\texttt{remote}) and \texttt{T5.4}~(\texttt{remote\_thread}) because the remote chain shares the target-process handle through the context.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}llll@{}}
        ...
        L2 & T2.3 & Remote alloc        & \texttt{CreateProcess} suspended + \texttt{VirtualAllocEx} \\
