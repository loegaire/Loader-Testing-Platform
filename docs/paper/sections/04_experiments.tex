...
\begin{itemize}[leftmargin=*, itemsep=1pt]
    ...
    \item \textbf{Target VM}: Windows~10 22H2, with Windows Defender enabled (definitions updated at test time) and Sysmon~\cite{sysmon} configured for loader-relevant events.
    ...
    \item \textbf{Security product}: Windows Defender only. Multi-product evaluation is scoped as future work (Section~5.3).
    \item \textbf{Execution}: Payloads run under a standard user account. A clean snapshot is restored before each test.
\end{itemize}

\subsection{Implemented Techniques}

Table~\ref{tab:techniques} lists the techniques currently implemented in the platform.
Two pairs of techniques are interdependent: \texttt{T2.1}~(\texttt{local}, RWX in one step) pairs with \texttt{T4.1}~(plain \texttt{memcpy}), while \texttt{T2.2}~(\texttt{local\_rw}, RW only) must be paired with \texttt{T4.2}~(\texttt{memcpy} followed by a flip to RX).
Other L2/L4 combinations leave the buffer in a non-executable state and will abort at L5.
...
Table~\ref{tab:results} reports detection outcomes against Windows Defender, covering all combinations of L3 (Transformation) and L5 (Execution) with the two fully implemented API modes.
...
\begin{table}[htbp]
    \centering
    \caption{Detection outcomes against Windows Defender, per L3/L5 configuration and API mode. L2 is fixed at \texttt{T2.1} (local RWX) with \texttt{T4.1} (\texttt{memcpy}); L0 is \texttt{T0.0} (none); L1 is \texttt{T1.1} (\texttt{.rdata}).}
    \label{tab:results}
    \small
    \begin{tabular}{@{}llcc@{}}
        \toprule
...
When static detection is evaded, direct syscalls bypass user-mode API hooks on allocation, writing, and thread creation, and the API mode is expected to become relevant in shifting outcomes from RD to S.
