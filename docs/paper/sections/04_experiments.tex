...
\begin{itemize}[leftmargin=*, itemsep=1pt]
    ...
    \item \textbf{Execution}: the loader process is launched at Medium integrity (a standard-user token, via Task Scheduler with \texttt{-RunLevel Limited}) to match a realistic initial-access scenario where the victim is a non-administrator. None of the implemented techniques---including the remote-injection chain T2.3/T2.4 + T4.3 + T5.4, which targets the user's own \texttt{explorer.exe} or a spawned \texttt{notepad.exe}---requires \texttt{SeDebugPrivilege} or administrator rights, because all cross-process operations stay within the same user. Administrative privileges are retained only for the harness itself (Sysmon configuration updates and Defender/Sysmon event log reading), which is a test-instrumentation concern separate from the loader.
    \item \textbf{Reset}: a clean snapshot is restored before each run; the VM network is isolated from the Internet to eliminate variability from Defender's cloud-delivered protection lookups.
