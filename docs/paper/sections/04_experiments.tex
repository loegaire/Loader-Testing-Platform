...
\begin{itemize}[leftmargin=*, itemsep=1pt]
    ...
    \item \textbf{Execution}: the loader process is launched at Medium integrity (a standard-user token, via Task Scheduler with \texttt{-LogonType Interactive -RunLevel Limited}) to match a realistic initial-access scenario where the victim is a non-administrator. Task Scheduler is used rather than a direct SSH \texttt{Start-Process} so that the payload runs in the logged-in user's interactive session rather than the sshd service session; this matters because remote-injection loaders that call \texttt{OpenProcess} on \texttt{explorer.exe} fail across session boundaries even when the same user is on both sides. None of the implemented techniques---including the remote-injection chain T2.3/T2.4 + T4.3 + T5.4---requires \texttt{SeDebugPrivilege} or administrator rights, because all cross-process operations stay within the same user. Administrative privileges are retained only for the harness (Sysmon configuration and event log reading), which is a test-instrumentation concern separate from the loader.
    \item \textbf{Reset and cloud isolation}: a clean snapshot is restored before each run. The guest is prepared by a one-time setup script that (i) enables auto-login so an interactive session exists at boot, (ii) sets Defender's \texttt{MAPSReporting} to \texttt{Disabled} and \texttt{SubmitSamplesConsent} to \texttt{NeverSend}, and (iii) isolates outbound connectivity at the libvirt network layer. Steps (ii) and (iii) are important for reproducibility: without them, we observed configurations that succeeded at time $t_1$ being blocked at $t_2$ within the same session, consistent with cloud-delivered signature updates.
